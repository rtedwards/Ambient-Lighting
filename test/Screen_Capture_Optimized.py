...
class ScreenShot(object):
    ...
    def averageColorInRegion(self,x1,y1,x2,y2,skip_factor):
        ...
        pixels = abs((x2-x1) * (y2-y1)) / skip_factor**2

        rgb_avg = np.zeros(4, dtype=np.int)
        rgb = np.zeros((pixels, 4), dtype=np.int)
        counter = 0 #count pixels read in

        for i in range(x1, x2-20, skip_factor):
            for j in range(y1, y2, skip_factor):
                #rgb += self.pixel(i, j)
                

                #use numpy sum or cumsum
                rgb[counter] = self.pixel(i, j)
                counter += 1
                #rgb[0] += temp[0]
                #rgb[1] += temp[1]
                #rgb[2] += temp[2]
        ...
        rgb_avg = rgb.sum(axis=0) / pixels  #calculate average
...
if __name__ == '__main__':
    ...
    right = np.zeros((right_lights,4), dtype=np.int)
    top = np.zeros((top_lights,4), dtype=np.int)
    left = np.zeros((left_lights,4), dtype=np.int)
    colors = np.zeros((lights,4), dtype=np.int)
    ...
    while (1):
        ...
        with timer("Averaging Colors in Sections"):
            ...
            colors = np.concatenate((right, top), axis=0)   #concatenate averages in order
            colors = np.concatenate((colors, left), axis=0)
